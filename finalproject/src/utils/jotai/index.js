/**
 * 죠-타이 (jotai)
 * - Recoil의 스타일을 계승하여 최신버전과의 호환성을 개선한 상태관리 라이브러리
 * - 대부분의 Recoil과 비슷하기 때문에 러닝커브 없이 마이그레이션 가능
 * - recoil에서는 atom, selector로 상태를 구현하지만 jotai는 atom만으로 생성
 * - atom(값)은 recoil atom와 같음
 * - atom(함수)는 recoil selector와 같음
 * 
 * Recoil에는 없지만 Jotai에는 있는 개발자 도구(DevTools)
 * - npm i jotai-devtools
 * - 프로그램 내부에 개발 모드에서만 작동하도록 <DevTools> 배치
 */
...
// export const clearLoginState = atom(null, // 읽는건 필요없고
//     (get, set)=>{ // 변경만 하겠다.
//             set(loginIdState, "");
//             set(loginLevelState, "");
